...
//
double Rect::GetCenterXY() const {
    double x = this->m_left, y = this->m_top, 
        x0 = x + (this->m_right - this->m_left) / 2.0,
        y0 = y + (this->m_bottom - this->m_top) / 2.0;  
...
std::ostream& operator<<(std::ostream& stream, const Rect& r) {
    stream << "Rect{l: "  << r.m_left << ", t: " << r.m_top << ", r: " << r.m_right << ", b: " << r.m_bottom << "}";
   // stream  << "++" << r.GetCenterXY() << "++";
    return stream;
}